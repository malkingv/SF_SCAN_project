...
function TokenRequest(userData, reset, handleNavigate,setIsError){

  // const fakeData = {
  //   accessToken: 12312312312,
  //   expire: '2023-06-12T00:20:24.6718511+03:00'
  // }
    // const login = {
    //     login: 'sf_student1',
    //     password: '4i2385j'
  ...
  fetch('https://gateway.scan-interfax.ru/api/v1/account/login', {
    ...
    body: JSON.stringify(userData),
        ...
        .then( (data) =>  {
            ...
            // store.dispatch(loginUser(fakeData))
...
const AuthBlock = () => {
  ...
  const onSubmit =  (data) => {
    TokenRequest(data, reset, handleNavigate, setIsError)
  }

  return(
    <AuthBlockWrapper onSubmit={handleSubmit(onSubmit)}>
      <div className={'auth'}>
      ...
      <div className={'btn same-opt'}>
        <Button name={isSubmitted ? 'Проверка данных...' : 'Войти'} size={22} height={26} type={'submit'} bg={isValid ? null: '#A4B0FF'}/>
      </div>
...
const AuthBlockWrapper = styled.form`
