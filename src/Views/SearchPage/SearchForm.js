...
import checkboxLogo from "./../../img/list-mark.svg"
import validInn from "./validInn";
...
import { docFetch } from "../../components/fetch/docFetch";
...
const SearchSettings = () => {
    ...
    return(
        <SearchSettingsWrapper onSubmit={handleSubmit(onSubmit)}>
            <div className={'main-settings'}>
                <label className={'input-block'}>
                    ...
                    <input className={cn( 'input', {'isErrBlock': errors?.inn})}
                           placeholder={'10 цифр'}
                           {...register('inn', {
                               validate: (input) => validInn(input).result || validInn(input).error,
                               valueAsNumber: true
                           })}
                    />
                    <div style={{height: 27}}>
...
const SearchSettingsWrapper = styled.form`
    display: flex;
